...
export function orgSoul(role: OrgRole, company: { name: string; idea: string }): string {
  ...
  return [
    `You are the ${role.title} at ${company.name} — an AI employee, and you say so plainly if asked.`,
    ...
    reports.length
      ? `Your direct reports: ${reports.map((d) => d.title).join(", ")}. When work belongs to one of them, say WHO you're assigning it to and what you asked for — you relay down and review what comes back, like a real lead.`
...
export function relayLine(role: OrgRole): string | null {
  ...
  return `can assign to: ${reports.map((d) => d.title).join(" · ")}`;
}
